"""Validate and apply the issue #24 Virginia Tech/Clemson pilot release."""
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError(f"cannot read issue #24 rows {path}: {error}") from error
    return rows
...
def _read_metadata(root: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"cannot read issue #24 metadata: {error}") from error
    if not isinstance(metadata, dict) or metadata.get("format_version") != FORMAT_VERSION:
        raise ValueError("unsupported issue #24 artifact format")
    return metadata


def _int(value: Any, field: str, row: Mapping[str, Any]) -> int:
    if isinstance(value, bool):
        raise ValueError(f"issue #24 {field} must be an integer: {row}")
    try:
        return int(value)
    except (TypeError, ValueError) as error:
        raise ValueError(f"issue #24 {field} must be an integer: {row}") from error
...
def _float(value: Any, field: str, row: Mapping[str, Any], *, nullable: bool = False) -> float | None:
    if value is None and nullable:
        return None
    if isinstance(value, bool):
        raise ValueError(f"issue #24 {field} must be numeric: {row}")
    try:
        number = float(value)
    except (TypeError, ValueError) as error:
        raise ValueError(f"issue #24 {field} must be numeric: {row}") from error
    if not math.isfinite(number):
        raise ValueError(f"issue #24 {field} must be finite: {row}")
    return number
...
def _validate_profile_rows(rows: list[dict[str, Any]], metadata: Mapping[str, Any]) -> tuple[list[dict[str, Any]], set[tuple[int, int]]]:
    ...
    for row in rows:
        ...
        if missing:
            raise ValueError(f"issue #24 profile row is missing {missing[0]}")
        ...
        if key in seen or version != MODEL_VERSION or zone not in ZONES:
            raise ValueError(f"duplicate, out-of-scope, or mismatched issue #24 profile row: {row}")
        if player_id <= 0 or season != SEASON:
            raise ValueError(f"issue #24 profile row is outside the locked scope: {row}")
        if attempts < 0 or makes < 0 or makes > attempts:
            raise ValueError(f"issue #24 profile counts are invalid: {row}")
        if attempt_share is None or not 0.0 <= attempt_share <= 1.0:
            raise ValueError(f"issue #24 attempt share is outside [0, 1]: {row}")
        if alpha is None or beta is None or alpha <= 0.0 or beta <= 0.0:
            raise ValueError(f"issue #24 posterior parameters are invalid: {row}")
        if adjusted_pps is None or not 0.0 <= adjusted_pps <= 3.0:
            raise ValueError(f"issue #24 adjusted PPS is outside [0, 3]: {row}")
        value = {
            "player_id": player_id,
            "season": season,
    ...
    if len(pairs) != 20:
        raise ValueError(f"issue #24 requires exactly 20 eligible player-seasons, got {len(pairs)}")
    if set(metadata.get("player_ids", [])) != {player_id for player_id, _season in pairs}:
        raise ValueError("issue #24 profile player IDs do not match metadata scope")
    for pair, pair_rows in grouped.items():
        if {row["zone"] for row in pair_rows} != set(ZONES):
            raise ValueError(f"issue #24 player-season does not have five zones: {pair}")
        if not math.isclose(sum(float(row["attempt_share"]) for row in pair_rows), 1.0, rel_tol=0.0, abs_tol=1e-8):
            raise ValueError(f"issue #24 attempt shares do not sum to one: {pair}")
...
def _validate_event_rows(rows: list[dict[str, Any]], metadata: Mapping[str, Any], pairs: set[tuple[int, int]]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    for row in rows:
        ...
        if missing:
            raise ValueError(f"issue #24 event row is missing {missing[0]}")
        ...
        if source_play_id <= 0 or source_play_id in seen:
            raise ValueError(f"duplicate or invalid issue #24 source_play_id: {row}")
        if version != MODEL_VERSION or season != SEASON or team_id not in TEAM_IDS or player_id not in player_ids or (player_id, season) not in pairs:
            raise ValueError(f"issue #24 event is outside the locked scope: {row}")
        if game_id <= 0 or status not in STATUSES:
            raise ValueError(f"issue #24 event identity/status is invalid: {row}")
        if not isinstance(row["made"], bool):
            raise ValueError(f"issue #24 event made value must be boolean: {row}")
        if row["shot_range"] is not None and not isinstance(row["shot_range"], str):
            raise ValueError(f"issue #24 event shot_range must be text or null: {row}")
        if row["shot_range"] == "free_throw":
            raise ValueError("issue #24 event artifact must contain field goals only")
        ...
        if basket not in {"left", "right", None}:
            raise ValueError(f"issue #24 attacking basket is invalid: {row}")
        if source_missing and status != "missing_coordinates":
            raise ValueError("issue #24 status precedence requires missing_coordinates")
        if source_invalid and status != "invalid_coordinates":
            raise ValueError("issue #24 status precedence requires invalid_coordinates")
        if basket_supplied and not source_missing and not source_invalid and basket is None and status != "unresolved_direction":
            raise ValueError("issue #24 valid coordinates without direction must be unresolved_direction")
        ...
        if status == "mapped":
            if source_missing or source_invalid or (basket_supplied and basket is None) or normalized_x is None or normalized_y is None or str(zone) not in ZONES:
                raise ValueError(f"issue #24 mapped event lacks valid evidence: {row}")
            if not 0.0 <= normalized_x <= 94.0 or not 0.0 <= normalized_y <= 50.0:
                raise ValueError(f"issue #24 mapped geometry is outside the court: {row}")
            reproduced = (
            ...
            if reproduced != zone:
                raise ValueError(f"issue #24 mapped zone does not reproduce geometry: {row}")
        else:
            if normalized_x is not None or normalized_y is not None or zone is not None:
                raise ValueError(f"issue #24 excluded event must not carry mapped evidence: {row}")
        value = {
...
def validate_artifact(root: Path, descriptor: dict[str, Any]) -> dict[str, Any]:
    ...
    for name in (PROFILE_FILE, EVENT_FILE, METADATA_FILE):
        if not (root / name).is_file():
            raise ValueError(f"issue #24 artifact is missing {name}")
    metadata = _read_metadata(root)
    if metadata.get("dataset") != "player_shot_event_locations":
        raise ValueError("issue #24 artifact dataset is invalid")
    if descriptor.get("release_version", RELEASE_VERSION) != RELEASE_VERSION:
        raise ValueError("issue #24 descriptor release version is not locked")
    if descriptor.get("model_version", MODEL_VERSION) != MODEL_VERSION:
        raise ValueError("issue #24 descriptor model version is not locked")
    if metadata.get("release_version") != RELEASE_VERSION or metadata.get("model_version") != MODEL_VERSION:
        raise ValueError("issue #24 artifact release/model version is invalid")
    ...
    if not isinstance(source_releases, dict) or source_releases.get("shot_corpus") != "issue-0006-shots-2026.1" or source_releases.get("audited_profiles") != "issue-0009-shot-location-2026.1":
        raise ValueError("issue #24 source release pins are missing or invalid")
    if source_releases.get("audited_profile_model_version", SOURCE_PROFILE_VERSION) != SOURCE_PROFILE_VERSION:
        raise ValueError("issue #24 audited profile model version is invalid")
    ...
    if source_versions is not None and (
        ...
        raise ValueError("issue #24 source release version aliases are invalid")
    ...
    if (
        ...
        raise ValueError("issue #24 source release checksums are missing or invalid")
    ...
    if not all(configuration.get(key) == value for key, value in expected_configuration.items()):
        raise ValueError("issue #24 model configuration is not the locked pilot configuration")
    if metadata.get("first_season") != SEASON or metadata.get("last_season") != SEASON:
        raise ValueError("issue #24 artifact season is not locked to 2026")
    if sorted(metadata.get("team_ids", [])) != sorted(TEAM_IDS) or any(isinstance(team, bool) or not isinstance(team, int) for team in metadata.get("team_ids", [])):
        raise ValueError("issue #24 artifact team scope is invalid")
    if (
        ...
        raise ValueError("issue #24 artifact must name 20 unique player IDs")
    ...
    if memberships is not None:
        if not isinstance(memberships, list) or len(memberships) != 20:
            raise ValueError("issue #24 artifact must name the 20 eligible roster memberships")
        for membership in memberships:
            if not isinstance(membership, dict):
                raise ValueError("issue #24 roster membership is malformed")
            ...
            if key in membership_keys or team_id not in TEAM_IDS or season != SEASON or minutes is None or minutes < MIN_ROTATION_MINUTES:
                raise ValueError("issue #24 roster membership is outside the locked scope")
    ...
    if not isinstance(source_sha, str) or len(source_sha) != 64 or any(char not in "0123456789abcdefABCDEF" for char in source_sha):
        raise ValueError("issue #24 source SHA-256 is missing or malformed")
    ...
    if membership_keys and {(player_id, season) for _team_id, player_id, season in membership_keys} != pairs:
        raise ValueError("issue #24 roster memberships do not match profile player-seasons")
    excluded = metadata.get("excluded_sub_rotation_players")
    if excluded is not None:
        if not isinstance(excluded, list) or len(excluded) != 7:
            raise ValueError("issue #24 must retain the seven sub-100-minute players as unavailable")
        for item in excluded:
            if not isinstance(item, dict) or item.get("availability") != "unavailable_below_100_minutes":
                raise ValueError("issue #24 excluded roster player is malformed")
            ...
            if team_id not in TEAM_IDS or season != SEASON or (
                ...
                raise ValueError("issue #24 excluded roster player is outside the locked scope")
    ...
    if not isinstance(metadata_coverage, dict) or any(
        ...
        raise ValueError("issue #24 mapped coverage diagnostics do not match event rows")
    ...
    if not isinstance(metadata_agreement, dict) or not math.isclose(
        ...
        raise ValueError("issue #24 two-/three-point diagnostics do not match event rows")
    if metadata_agreement.get("comparable_mapped_events", comparable) != comparable or metadata_agreement.get("agreements", agreements) != agreements:
        raise ValueError("issue #24 two-/three-point diagnostic counts do not match event rows")
    ...
    if not isinstance(metadata_zones, dict) or any(int(metadata_zones.get(zone, -1)) != count for zone, count in zone_counts.items()):
        raise ValueError("issue #24 zone diagnostics do not match event rows")
    counts = {
    ...
    if not isinstance(metadata_row_counts, dict) or any(key not in metadata_row_counts for key in counts):
        raise ValueError("issue #24 metadata is missing exact row/status counts")
    for key, value in counts.items():
        expected = _expected_count(metadata, key)
        if expected is not None and expected != value:
            raise ValueError(f"issue #24 metadata mismatch for {key}: expected {expected}, got {value}")
        expected = descriptor.get("expected", {}).get("row_counts", {}).get(key)
        if expected is not None and int(expected) != value:
            raise ValueError(f"issue #24 descriptor row-count mismatch for {key}")
    expected_events = 3857
    if counts["event_locations"] != expected_events:
        raise ValueError(f"issue #24 requires exactly {expected_events} event rows, got {counts['event_locations']}")
    ...
    if not isinstance(coverage, dict) or any(float(coverage.get(str(team), -1)) < 0.90 for team in TEAM_IDS):
        raise ValueError("issue #24 mapped coverage is below 90 percent for a scoped team")
    agreement = metadata.get("two_three_point_agreement", {})
    if not isinstance(agreement, dict) or float(agreement.get("agreement", -1)) < 0.99:
        raise ValueError("issue #24 two-/three-point agreement is below 99 percent")
    ...
    if activation_gate is not None and (
        ...
        raise ValueError("issue #24 activation gate is not satisfied")
    ...
    if status_counts is not None and status_counts != statuses:
        raise ValueError("issue #24 status counts do not match event rows")
    return {
        "metadata": metadata,
        "profiles": profiles,
        "events": events,
...
def _verify_source_dependencies(conn: Any, descriptor: Mapping[str, Any]) -> None:
    for dependency in descriptor.get("source_dependencies", []):
        if not isinstance(dependency, Mapping):
            raise ValueError("issue #24 source dependency is malformed")
        with conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT status
        ...
        if row is None or row[0] != "published":
            raise ValueError(
                "issue #24 requires published source release "
...
def _check_source_events(conn: Any, events: list[dict[str, Any]], player_ids: set[int]) -> None:
    ...
    if source_ids != artifact_ids:
        raise ValueError(f"issue #24 source event completeness mismatch: missing={sorted(source_ids-artifact_ids)[:10]}, extra={sorted(artifact_ids-source_ids)[:10]}")
    ...
    for event in events:
        ...
        if expected[:-2] != actual[:-2] or not _same_source_coordinate(expected[-2], actual[-2]) or not _same_source_coordinate(expected[-1], actual[-1]):
            raise ValueError(f"issue #24 source event changed for {event['source_play_id']}")
...
def _check_source_profiles(conn: Any, profiles: list[dict[str, Any]], player_ids: set[int]) -> None:
    ...
    if set(expected) != set(actual):
        raise ValueError("issue #24 audited source profile completeness mismatch")
    for key, row in actual.items():
        ...
        if values != expected_values:
            raise ValueError(f"issue #24 audited profile changed for {key}")
...
def _check_source_directions(conn: Any, events: list[dict[str, Any]]) -> None:
    ...
    for event in events:
        ...
        if basket is not None and directions.baskets.get(int(event["source_play_id"])) != basket:
            raise ValueError(f"issue #24 source game direction changed for {event['source_play_id']}")
...
def apply(conn: Any, root: Path, descriptor: dict[str, Any], prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    if Json is None or execute_values is None:
        raise RuntimeError("psycopg2 is required to apply issue #24")
    ...
    if actual_pairs != pairs:
        raise ValueError(f"issue #24 eligibility mismatch: missing={sorted(actual_pairs-pairs)[:20]}, extra={sorted(pairs-actual_pairs)[:20]}")
    if metadata.get("eligible_roster_memberships") is not None:
        ...
        if actual_memberships != expected_memberships:
            raise ValueError(f"issue #24 roster-membership mismatch: missing={sorted(actual_memberships-expected_memberships)[:20]}, extra={sorted(expected_memberships-actual_memberships)[:20]}")
    ...
    if not isinstance(configuration, dict):
        raise ValueError("issue #24 model configuration is missing")
    expected_configuration = {
        "model_version": MODEL_VERSION,
        "base_model_version": SOURCE_PROFILE_VERSION,
        "zone_scheme": list(ZONES),
    ...
    with conn.cursor() as cursor:
        ...
        if existing is None:
            ...
            raise ValueError("issue #24 model configuration conflicts with the database")
        ...
        for row in profiles:
            ...
            if key in existing_profiles and not _rows_equal(existing_profiles[key], row, ("attempts", "makes", "attempt_share", "posterior_alpha", "posterior_beta", "adjusted_pps")):
                raise ValueError(f"issue #24 conflicting existing profile row: {key}")
        ...
        for row in events:
            ...
            if key in existing_events:
                ...
                if any(current[field] != row[field] for field in ("mapping_status", "normalized_x", "normalized_y", "zone")):
                    raise ValueError(f"issue #24 conflicting existing event row: {key}")
        ...
        if cursor.rowcount != 1:
            raise RuntimeError("issue #24 scoped model version was not activated")
